...
title('confidence interval of truncated variance in two weeks of PG');
box off; grid on;
xlabel('time');
ylabel('CI in two weeks');
print(f,'-dpng','-r200','figures/1C2');
close(f);
...
datetick('x','mmdd');
legend('TV_zoom','CI_zoom');
title('confidence interval of annualized truncated variance in two weeks of PG');
box off; grid on;
xlabel('time');
ylabel('CI in two weeks');
print(f,'-dpng','-r200','figures/1E2');
close(f);
